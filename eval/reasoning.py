...
def eval(model_name, task_names):
    ...
    for task_name in task_names:
        ...
        if os.path.isfile(csv_file):
            ...
            with open(csv_file, mode='r', encoding='utf-8') as f:
                ...
                for row in reader:
                    recorded_model = row.get("model_name", "")
                    recorded_task = row.get("task_name", "")
                    if recorded_model == model_name.split('/')[-1] and recorded_task == task_name:
                        print(f"Already evaluated: model={model_name}, task={task_name}. Skipping.")
                        exist_ = True
                        break
            if exist_:
                continue
        ...
        if task_name == "aime90":
            ...
            raise NotImplementedError(f"{task_name} not implement yet")
